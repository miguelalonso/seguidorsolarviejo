...
float angulo_FC_este= -120.0;
float angulo_FC_norte=  2.0;
 ...
 // lcd.print(F("Inicializando:"));
  ...
  FC_este = digitalRead(FC_este_pin);
    ...
    Serial.println(F("Buscando final carrera Norte:")); //gira al Norte
      ...
      cenit_anterior=angulo_FC_norte; 
      EncZ.write(0);
      iniciado_Norte=true;
    }
    ...
    lcd.print(EncA.read());
      ...
      acimut_anterior=angulo_FC_este;
      EncA.write(0);
      iniciado_Este=true;
    } 
...
/// función para seguir al sol
  ...
  if (abs(acimut_sol)>acimut_max && acimut_sol<0){acimut_sol=-acimut_max;} else{acimut_sol=acimut_sol;}
    ...
    if (abs(grados_acimut) > precision_acimut){
      if(grados_acimut<10){ //para evitar el inicio
        grados_acimut=1.5*abs(grados_acimut); // se mueve el doble de lo necesario, la mitad de movimientos
      }else{
        grados_acimut=abs(grados_acimut);
      }
   ...
   //seguimiento cenital
    ...
    grados_cenit=(cenit_sol-cenit_actual);
     if (abs(grados_cenit) > precision_cenit){
        if(grados_cenit<5){
         grados_cenit=1.5*abs(grados_cenit); // se mueve el doble de lo necesario, la mitad de movimientos 
        }else{
          grados_cenit=abs(grados_cenit);
        }
        pulsos_cenit=grados_cenit/grados_pulso_cenit;
        
        if (cenit_sol>cenit_actual) {
          gira_Sur(1);
          cenit_temp=cenit_actual+abs(EncZ.read())*grados_pulso_cenit; 
          if (abs(EncZ.read())>pulsos_cenit){
            gira_Sur(0);
            delay(1000);
            cenit_actual=cenit_actual+abs(EncZ.read())*grados_pulso_cenit; 
            cenit_anterior=cenit_actual; 
            EncZ.write(0);
            return;
            }
          }
...
void display_anguloactual(){
  lcd.setCursor(0,1);
  lcd.print(F("aZ:"));
  if (estado_giroN || estado_giroS){
    lcd.print(cenit_temp);
  }else{
    lcd.print(cenit_actual);
  }
  
  lcd.print(F("aA:"));
  if (estado_giroE || estado_giroO){
    lcd.print(acimut_temp);
  }else{
